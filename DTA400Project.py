...
pastries_sold = 0
missed_pastry_selling_opportunity = 0
...
class customer(object): #make a function??
    def __init__(self, env):
        ...
        for item in menu: #randomize order
            wanted_amount = random.randint(0,3)
        ...
        if num_pastries < 1: #make sure the customer wants at least 1 thing
            ...
            self.order[random.randint(0,len(menu)-1)][1] = random.randint(1,3)
...
env.run(until=10)  #end when either time over or every part of menu is <= 0